...
def connectAndListen(ser, ackCommand, listenTimeout=CONNECT_LISTEN_TIMEOUT):
    ...
    #
    # Buffer is cleared BEFORE resetting, not after. resetK4H818() takes
    # real wall-clock time (subprocess calls, explicit settle delays),
    # and the AVR's reset actually happens partway through its
    # execution -- clearing the buffer after resetK4H818() returns can
    # discard "K4H818READY" if it already arrived and was sitting in
    # the buffer by then, while nothing else is lost since nothing
    # later has been transmitted yet. This exactly matches the observed
    # symptom: only the very first line ever went missing.
    ser.reset_input_buffer()

    connect_t0 = time.time()
    reset_start = time.time() - connect_t0
    resetK4H818(ser)
    reset_end = time.time() - connect_t0
    if verboseMode:
        print("DIAG: reset_input_buffer() at t=0.00s, resetK4H818() ran from t=" +
              "{:.3f}".format(reset_start) + "s to t=" + "{:.3f}".format(reset_end) + "s")
